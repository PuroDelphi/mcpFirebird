...
async function getEventManager(): Promise<any> {
    ...
    try {
        ...
        if (driverInfo.current === DriverType.NATIVE) {
            ...
            if (!attachment) {
                throw new Error("Native driver attachment not found");
            }
            logger.info('Using native driver for events');
            
            fbEventManager = {
                attachment,
                eventsObj: null as any,
                registerEvent: async (events: string[], callback: (err: any) => void) => {
                    try {
                        if (fbEventManager.eventsObj) {
                            await fbEventManager.eventsObj.cancel();
                        }
                        fbEventManager.eventsObj = await attachment.queueEvents(events, async (counters: [string, number][]) => {
                            for (const [name, count] of counters) {
                                logger.info(`🔥 FIREBIRD EVENT FIRED (Native): ${name} (count: ${count})`);
                ...
                close: async (callback: () => void) => {
                    if (fbEventManager.eventsObj) {
                        try {
                            ...
                            logger.error("Error cancelling native events", { error: e });
                        }
                    }
                    callback();
                }
            ...
            // Using pure JS driver
            const targetDb = (db as any)._nativeAttachment || db;
...
export async function registerFirebirdEvents(events: string[]): Promise<void> {
    ...
    const allEvents = Array.from(new Set([...Array.from(activeEvents.keys()), ...events]));
    
    return new Promise((resolve, reject) => {
        evtManager.registerEvent(allEvents, (err: any) => {
            if (err) {
                logger.error(`Failed to register events: ${allEvents.join(', ')}`, { error: err });
...
/**
 * Configures the MCP server to handle Firebird events as resources and tools.
...
export function setupEventResources(server: any) {
    ...
    if (typeof server.registerResource === 'function') {
        ...
        server.registerTool(
            "subscribe_to_event",
            {
                title: "subscribe_to_event",
                description: "Subscribe to a Firebird event (POST_EVENT) to receive proactive notifications",
        ...
        logger.info('Registered Firebird Event tools and resources on modern McpServer');
        
    } else {
        // Legacy implementation support (fallback)
        server.resource(
            "firebird-event",
    ...
    if (fbEventManager && typeof fbEventManager.close === 'function') {
        fbEventManager.close(() => {
            if (eventDbConnection) {
                // If it's a native connection, maybe we need extra cleanup?
                // For now, rely on detach()
                if (typeof eventDbConnection.detach === 'function') {
